...
export default function CheckoutPage() {
  ...
  useEffect(() => {
    ...
    if (amount < 100) {
      setErrorMessage('Cart is empty or amount too low');
      setStatus('error');
      return;
    }
      ...
      <div className="min-h-screen bg-gradient-to-br from-green-50 to-emerald-100">
        ...
        <div className="flex items-center justify-center p-4 min-h-[80vh]">
          ...
          >
            ...
            <p className="text-gray-600 mb-6">Your payment of Rs {amount.toLocaleString()} has been processed successfully.</p>
            <Link
              href="/"
              className="inline-block py-3 px-6 bg-[#1E88E5] text-white font-semibold rounded-xl hover:bg-[#1565C0] transition-all"
            >
              Continue Shopping
            </Link>
          </motion.div>
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        <div className="grid lg:grid-cols-2 gap-8">
          ...
          >
            ...
            {clientSecret && !isManualPayment && status === 'idle' && (
              <motion.div
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                className="bg-white rounded-2xl shadow-lg p-6"
            ...
            <div className="bg-white rounded-2xl shadow-lg p-6">
                ...
                {items.map((item) => (
                  <div key={item.id} className="flex gap-3 p-3 bg-gray-50 rounded-xl">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="font-bold text-[#1E88E5] text-sm mt-1">Rs {(item.price * item.quantity).toLocaleString()}</p>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="border-t border-gray-200 pt-4 space-y-3">
                ...
                <div className="border-t border-gray-200 pt-3 flex justify-between">
                  ...
                  <span className="text-lg font-bold text-[#1E88E5]">Rs {totalPrice.toLocaleString()}</span>
                </div>
              </div>
